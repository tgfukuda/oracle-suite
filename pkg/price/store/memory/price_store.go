...
type PriceStore struct {
	...
	prices map[store.FeederPrice]*messages.Price
}

// NewPriceStore creates a new store instance.
func NewPriceStore() *PriceStore {
	return &PriceStore{
		prices: make(map[store.FeederPrice]*messages.Price),
	}
}
...
func (p *PriceStore) All() map[store.FeederPrice]*messages.Price {
	p.mu.Lock()
	defer p.mu.Unlock()

	r := map[store.FeederPrice]*messages.Price{}
